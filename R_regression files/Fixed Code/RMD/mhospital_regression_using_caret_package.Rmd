...
```{r}
new.data$BMI <- new.data$BODY.WEIGHT/((new.data$BODY.HEIGHT/10) ^ 2)
new.data$I_COST.OF.IMPLANT <- model.matrix(~new.data$IMPLANT.USED..Y.N.)[,2]*new.data$COST.OF.IMPLANT
...
set.seed(2341)
trainIndex <- createDataPartition(filter.data$TOTAL.COST.TO.HOSPITAL, p = 1, list = FALSE)
...
set.seed(766)
#levels(reg.train.data$Status) <- make.names(levels(factor(reg.train.data$Status)))
regCaretModel <- train(reg.train.data[,1:14],
                      reg.train.data[,15], 
...
plot(varImp(regCaretModel, scale = TRUE))
```
...
plot(predict(regCaretModel$finalModel), residuals(regCaretModel$finalModel), main = "Scale-Location")
...
RegTestPrediction = predict(regCaretModel$finalModel, reg.test.data, 
                            interval = "confidence",
                            level = 0.95,
                            type = "response")
data.frame(RegTestPrediction, reg.test.data$TOTAL.COST.TO.HOSPITAL)
```
