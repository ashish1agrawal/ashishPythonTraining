...
dt.train.data <- as.data.frame(data.train[,c("DOJ.Extended",
...
dt.test.data <- as.data.frame(data.test[,c("DOJ.Extended",
                                           "Duration.to.accept.offer",
                                           "Notice.period",
                                           "Offered.band",
                                           "Percent.difference.CTC",
...
#dt.train.data$StatusFactor <- as.factor(ifelse(dt.train.data$Status == "Joined", 1,0))
set.seed(766)
levels(dt.train.data$Status) <- make.names(levels(factor(dt.train.data$Status)))
formula <- as.formula(dt.train.data$Status~.)
dtCaretModel <- train.formula(formula,
                      dt.train.data,
...
summary(dtCaretModel$finalModel)
#dev.off()
fancyRpartPlot(dtCaretModel$finalModel)
```
...
levels(dt.train.data$Status) <- make.names(levels(factor(dt.train.data$Status)))
caretPredictedClass <- predict(object = dtCaretModel, dt.train.data[,1:12], type = 'raw')
confusionMatrix(caretPredictedClass,dt.train.data$Status)
```
...
levels(dt.test.data$Status) <- make.names(levels(factor(dt.test.data$Status)))
dtCaretTestPredictedClass = predict(dtCaretModel, dt.test.data, type = "raw")
confusionMatrix(dtCaretTestPredictedClass,dt.test.data$Status)
```
...
#error in below line
dtCaretTestPredictedProbability = predict(dtCaretModel, dt.test.data, type = "prob")
dtPredObj <- prediction(dtCaretTestPredictedProbability[2],dt.test.data$Status)
